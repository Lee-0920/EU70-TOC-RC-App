...
void TempControl_SetCalibrateFactor(DscpDevice* dscp, Byte* data, Uint16 len)
{
    ...
    {
    	memcpy(&index, data, sizeof(Uint8));
        memcpy(&calibratefactor, data + sizeof(Uint8), sizeof(TempCalibrateParam));
        ThermostatManager_SetCalibrateFactor(index, calibratefactor);
    }
...
//	temperature.environmentTemp = TempCollecterManager_GetEnvironmentTemp();
    temperature.environmentTemp = ThermostatManager_GetCurrentTemp(1);
//        temperature.environmentTemp = ThermostatManager_GetCurrentTemp(1);
    // 发送回应
        ...
        Uint16 len)
{
    ...
    // 数据正确性判断
    size = sizeof(level) + sizeof(index);
    if ((len > size))
    {
        ret = DSCP_ERROR;
        TRACE_ERROR("Parame Len Error\n");
    ...
    if (DSCP_OK == ret)
    {
       memcpy(&index, data, sizeof(float));
	   memcpy(&level, data + sizeof(index), sizeof(float));
       BoxFanDriver_SetOutput(index, level);
    }
        ...
        Uint16 len)
{
    ...
    unsigned short ret = DSCP_OK;   // 默认系统正常
    // 数据正确性判断
    size =  sizeof(index) + sizeof(level);
    if ((len > size))
    {
        ret = DSCP_ERROR;
        TRACE_ERROR("Parame Len Error\n");
    ...
    {
		memcpy(&index, data, sizeof(Uint8));
        memcpy(&level, data + sizeof(Uint8), sizeof(float));
        TRACE_INFO("\n FanSetOutput  No. %d fan index, level %f.", index, level);
//       ThermostatManager_SetSingleRefrigeratorOutput(0, index, level);
        if(3 == index || 2 == index)
        {
        	ThermostatDeviceManager_SetOutput(index, level);
        }
    }
 ...
 * @note 该命令已不支持
 */
void TempControl_RelayControl(DscpDevice* dscp, Byte* data, Uint16 len)
{
    Uint16 ret = DSCP_OK;
    uint8_t status = 0;
    int size;
	...
	if (DSCP_OK == ret)
	{
		...
		if(status == 0)
		{
			Thermostat_RelayControl(FALSE);
		}
		else
		{
			Thermostat_RelayControl(TRUE);
		}
	}
 ...
 * @param level float ,风扇速率，对应高电平占空比。默认为0，恒温器风扇关闭。
 */
void TempControl_ThermostatFanOpen(DscpDevice* dscp, Byte* data,
        Uint16 len)
{
    Uint8 index;
    Uint8 size = 0;
    ...
    if(ret == DSCP_OK)
    {
        ...
        memcpy(&index, data, sizeof(Uint8));
        TRACE_INFO("\n open  No. %d fan,.", index);
    	ThermostatManager_SetSingleRefrigeratorOutput(0, index, 1);
    }
    DscpDevice_SendStatus(dscp, ret);// 发送状态回应
}
...
/**
 * @brief 修改恒温器参数。
 * @details 修改恒温器的恒温值,停掉恒温器，以新的参数开始，带索引值
 * @return 状态回应，Uint16，支持的状态有：
 *  - @ref DSCP_OK  操作成功；
 *  - @ref DSCP_ERROR 操作失败；
 * @note 该命令将立即返回。
 */
void TempControl_ReviseThermostatTemp(DscpDevice* dscp, Byte* data, Uint16 len)
{
	Uint8 index = 0;			//恒温器索引
    ThermostatMode mode;         //恒温
    float targetTemp;           // 目标温度
    ...
    unsigned short ret = DSCP_OK;
    //设置数据正确性判断
    size =  sizeof(Uint8) + sizeof(ThermostatMode) + sizeof(float) * 3;
    if ((len > size))
    {
        ret = DSCP_ERROR;
        TRACE_ERROR("Parame Len Error\n");
    ...
    {
        //修改并保存
    	memcpy(&index, data, sizeof(ThermostatMode));
		size = sizeof(index);
		...
		if(THERMOSTAT_BUSY == ThermostatManager_GetStatus(index))
		{
			if(FALSE == ThermostatManager_RequestStop(index))
			{
				ret = DSCP_ERROR;
			}
			else
		...
		TRACE_INFO("\nRevise Thermostat[%d]: targgetTemp %f, toleranceTemp %f, timeout %f\n",index,targetTemp,toleranceTemp,timeout);
