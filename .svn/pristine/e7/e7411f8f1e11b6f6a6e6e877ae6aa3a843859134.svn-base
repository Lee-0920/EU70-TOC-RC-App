...
static Uint32 s_valvesNormalOpen = 0;
...
{
	Uint32 i = 1;
	if(map > (i<<SOLENOIDVALVECONF_TOTALVAlVES))
	{
		TRACE_INFO("\nSet Normal Open Failed %x", map);
		return FALSE;
	}
	else
	{
		s_valvesNormalOpen = map;
		TRACE_INFO("\nSet Normal Open %x", s_valvesNormalOpen);
		return TRUE;
	}
