    ...
    while (1)
    {
        ...
        for(int i = 0; i < PUMPMANAGER_TOTAL_PUMP; i++)
        {
            if (TRUE == s_pumpEvents[i].isSendEvent)
            {
                ...
                TRACE_INFO("\n pump %d Envent: ", i);
			   if(PUMP_RESULT_FINISHED == s_pumpEvents[i].result)
			   {
				TRACE_INFO("PUMP_RESULT_FINISHED");
			   }
			   else if(PUMP_RESULT_FAILED == s_pumpEvents[i].result)
			   {
				TRACE_INFO("PUMP_RESULT_FAILED");
			   }
			   else
			   {
				TRACE_INFO("PUMP_RESULT_STOPPED");
			   }
            }
        }
