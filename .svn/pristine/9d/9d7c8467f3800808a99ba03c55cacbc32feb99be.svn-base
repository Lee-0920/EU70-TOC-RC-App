...
#define  EXTPUMP_OFFSET   4  //扩展泵的泵号相对Flash存储位置偏移
...
void Pump_Init(Pump* pump)
{
    ...
    pump->otherMoveHandler = NULL;
    pump->finishHandler = NULL;
}
...
Bool Pump_Start(Pump* pump, Direction dir, float volume, ParamFLashOp flashOp, Pump_OtherMoveHandler otherMoveHandler)
{
    if ( FALSE == PumpTimer_RegisterHandle((TimerHandle) Pump_MoveHandler, pump))
    {
        return FALSE;
    }
    pump->otherMoveHandler = otherMoveHandler;
...
Bool Pump_ImmediatelyStop(Pump* pump)
{
	TRACE_INFO("\n ###########StepperMotor %d", pump->number);
	TRACE_INFO("\n ###########StepperMotor_ImmediatelyStop");
	pump->result = (enum PumpResult)RESULT_STOPPED;
	Pump_MoveStatusSwitch(pump, MOVE_STATUS_IDLE);
	Pump_Stop(pump);
	if (NULL != pump->finishHandler)
	{
		pump->finishHandler(pump);
	}
	return TRUE;
}
...
void Pump_ChangeStep(Pump* pump, Uint32 step)
{
    Uint32 lastStep = pump->alreadyStep / pump->subdivision;
    Printf("\n alreadyStep=%d, change %d", lastStep, step);
    if (step > lastStep)
    {
    	pump->remainStep =
                (Uint32) (step * pump->subdivision) - pump->alreadyStep;
        TRACE_INFO("\n ###########ChangeStep remainStep: %d step", pump->remainStep);
        Pump_MoveStatusSwitch(pump, MOVE_STATUS_START);
        TRACE_INFO("\n ###########MoveStatusSwitch to START");
    }
    else
    {
    	Pump_RequestStop(pump);
...
void Pump_MoveHandler(Pump* pump)
{
    ...
    if (pump->otherMoveHandler != NULL)
	{
		if (0 == (pump->alreadyStep % pump->subdivision) && pump->alreadyStep != 0)
		{
			pump->otherMoveHandler(pump);
		}
	}
}
...
Bool Pump_ReadDiagnostic(Pump *pump)
{
	return PumpDriver_ReadDiagnostic(&pump->driver);
}

void Pump_RegisterFinishHandle(Pump* pump, Pump_FinishHandler handler)
{
	pump->finishHandler = handler;
}
