...
/**
 * @brief 查询所有泵的工作状态。
 * @return 状态回应，Uint16 map，支持的状态有：
 *  - @ref 0 空闲；
 *  - @ref 1 忙碌，需要停止后才能做下一个动作；
 */
#define DSCP_CMD_PPI_GET_TOTAL_PUMP_STATUS                (DSCP_PPI_CBASE + 0x0A)
 ...
 *  - @ref 1 忙碌，传感器被遮挡。
 */
#define DSCP_CMD_PPI_GET_TOTAL_SENSOR_STATUS           (DSCP_PPI_CBASE + 0x0B)

/**
 * @brief 设置指定泵每一个脉冲的体积ml。
 * @param index Uint8，要查询的泵索引，0号泵为光学定量泵。
 * @param volume 泵出的体积，Float32，单位为 ml。
 * @return 状态回应，Uint16，支持的状态有：
 *  - @ref DSCP_OK  操作成功；
 *  - @ref DSCP_ERROR 操作失败；
 *  - @ref DSCP_ERROR_PARAM 参数错误，传入的参数有问题
 */
#define DSCP_CMD_PPI_SET_PUMP_OFFSET_STEP                (DSCP_PPI_CBASE + 0x0C)

/**
 * @brief 获取指定泵每一个脉冲的体积ml。
 * @param index Uint8，要查询的泵索引，0号泵为光学定量泵。
 * @return 泵出的体积，Float32，单位为 ml。
 */
#define DSCP_CMD_PPI_GET_PUMP_OFFSET_STEP                  (DSCP_PPI_CBASE + 0x0D)
