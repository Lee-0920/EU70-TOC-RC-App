...
typedef enum
{
    SUCK,                   //抽
    DRAIN,                  //排
    MAX_OPERATE_DIRECTION
} OperateDirection;

/**
 * @brief 泵的安装类型
 * @detail 决定电机转动方向与实际泵的抽排方向的关系
 */
typedef enum
{
    CLOCKWISE_DRAIN_ANTICLOCKWISE_SUCK,    //安装类型:顺排逆抽
    CLOCKWISE_SUCK_ANTICLOCKWISE_DRAIN,     //安装类型:顺抽逆排
    MAX_FIXED_TYPE
}FixedType;

typedef void (*Pump_OtherMoveHandler)(void *pump);//步进电机每走一步（未细分）的附加处理
typedef void (*Pump_FinishHandler)(void *pump);//步进电机完全停止的附加处理
...
typedef struct
{
    ...
    Pump_OtherMoveHandler otherMoveHandler;
	Pump_FinishHandler finishHandler;
} Pump;
