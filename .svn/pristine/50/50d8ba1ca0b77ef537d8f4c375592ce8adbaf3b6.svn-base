...
static void PumpManager_FinishHandler(void *obj);
...
//    //XZ轴注册结束回调函数
//    Pump_RegisterFinishHandle(&g_pumps[6], PumpManager_FinishHandler);
//    Pump_RegisterFinishHandle(&g_pumps[7], PumpManager_FinishHandler);
    PumpEventScheduler_Init();
}

void PumpManager_FinishHandler(void *obj)
{
//    Pump *pump = (Pump *)obj;
//    Uint8 number = pump->number;
//    if(TRUE == g_pumps[number].isSendEvent)
//    {
//        enum PumpResult pumpResult = (enum PumpResult)pump->result;
//        Uint8 data[2] = {0};
//        data[0] = number;
//        data[1] = pumpResult;
//        TRACE_INFO("\n Pump %d Finish Without Event", number);
////        PumpEventScheduler_SendEvent(data[0], pumpResult, TRUE);
////        DncpStack_BufferEvent(DSCP_EVENT_PPI_PUMP_RESULT, data , sizeof(data));
//    }
//    g_pumps[number].isSendEvent = FALSE;
//    float alreadyVolume = (float) (Pump_GetAlreadyStep(pump) * g_pumps[number].factor);
        ...
        float maxSpeed, ParamFLashOp flashOp)
{
    ...
    TRACE_INFO("\n Pump limit speed %f", (PUMP_MAX_SPEED * g_pumps[index].factor));
...
Uint16 PumpManager_Start(Uint8 index, Direction dir, float volume, ParamFLashOp flashOp)
{
    if (PUMP_IDLE == g_pumps[index].status)
    {
        if (index < PUMPMANAGER_TOTAL_PUMP && volume > 0 && dir < MAX_DIRECTION)
        {
            ...
            if  (TRUE == Pump_Start(&g_pumps[index], dir, volume, flashOp, NULL))
            {
                return DSCP_OK;
            }
            else
...
Bool PumpManager_SetSubdivision(Uint8 index, Uint8 subdivision)
{
    if (index < PUMPMANAGER_TOTAL_PUMP)
    {
        Pump_SetSubdivision(&g_pumps[index], subdivision);
        return TRUE;
    }
    else
    {
