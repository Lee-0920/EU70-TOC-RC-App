...
const static ThermostatParam kStoveThermostatParam =
{ .proportion = 70, .integration = 0.2, .differential = 180, };
...
void ThermostatManager_Init()
{
    ...
    Thermostat_SetTemp(&s_thermostats[0], MEASUREMODULE1_TEMP);		//设置监视的温度索引
    index = MEASUREMODULE_HEATER1;   //设置对应的温控设备索引 0-5
	...
	Thermostat_SetHeater(&s_thermostats[1], 0, &index);
...
char* ThermostatManager_GetName(Uint8 index)
{
    ...
    {
    case 0:
        strcpy(name, "Measure Stove");
        break;
    case 1:
        strcpy(name, "Measure Cooler");
        break;
    case 2:
		strcpy(name, "Measure LNDIR");
		break;
    case 3:
		strcpy(name, "Measure SNDIR");
		break;
    default:
        strcpy(name, "NULL");
        break;
...
//        	temp = 22.5;
            ...
            temp = TempCollecterManager_GetTemp(MEASUREMODULE1_TEMP);
//            temp = 23.5;
			memcpy(&data[4], &temp, sizeof(temp));
            temp = TempCollecterManager_GetTemp(MEASUREMODULE3_TEMP);
//			temp = 24.5;
            memcpy(&data[8], &temp, sizeof(temp));
			temp = TempCollecterManager_GetTemp(MEASUREMODULE4_TEMP);
//            temp = 25.5;
...
Bool ThermostatManager_SetSingleRefrigeratorOutput(Uint8 thermostatIndex, Uint8 fanIndex, float level)
{
    ...
    {
    	TRACE_INFO("\n SetOutput  No. %d fan index.", fanIndex);
...
TempCalibrateParam ThermostatManager_GetCalibrateFactor(Uint8 index)
{
    ...
    else if(index < TOTAL_TEMP)
    {
    	 tempCalibrateParam = TempCollecterManager_GetCalibrateFactor(index);
    }
    else
    {
...
Bool ThermostatManager_SetCalibrateFactor(Uint8 index, TempCalibrateParam tempCalibrateParam)
{
	...
	{
    	TRACE_INFO("\n Set Extra Temperature %d", index);
    	ret = TempCollecterManager_SetCalibrateFactor(index, tempCalibrateParam);
	}
    else
    {
