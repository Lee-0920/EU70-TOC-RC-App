...
//#define PUMP_TIMERE_RCC                RCC_APB1Periph_TIM6
//#define PUMP_TIMERE_RCC_CONFIG         RCC_APB1PeriphClockCmd(PUMP_TIMERE_RCC,ENABLE)
//#define PUMP_TIMERE_IRQn               TIM6_DAC_IRQn
//#define PUMP_TIMERE_IRQHANDLER         TIM6_DAC_IRQHandler
//#define PUMP_TIMERE                    TIM6

//#define PUMP_TIMERF_RCC                RCC_APB1Periph_TIM14
//#define PUMP_TIMERF_RCC_CONFIG         RCC_APB1PeriphClockCmd(PUMP_TIMERF_RCC, ENABLE)
//#define PUMP_TIMERF_IRQn               TIM8_TRG_COM_TIM14_IRQn
//#define PUMP_TIMERF_IRQHANDLER         TIM8_TRG_COM_TIM14_IRQHandler
//#define PUMP_TIMERF                    TIM14
...
static void (*PumpTimerE_Handle)(void *obj) = NULL;
static void (*PumpTimerF_Handle)(void *obj) = NULL;
...
static void* s_timerEObj = NULL;
static void* s_timerFObj = NULL;
...
//    NVIC_InitStructure.NVIC_IRQChannel = PUMP_TIMERE_IRQn;
//	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = PUMP_TIMERD_PRIORITY;
//	NVIC_Init(&NVIC_InitStructure);

//	NVIC_InitStructure.NVIC_IRQChannel = PUMP_TIMERF_IRQn;
//	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = PUMP_TIMERD_PRIORITY;
//	NVIC_Init(&NVIC_InitStructure);
}
...
//    PUMP_TIMERE_RCC_CONFIG;
//    PUMP_TIMERF_RCC_CONFIG;
}
...
//    TIM_TimeBaseStructure.TIM_Period = PUMP_TIMER_PERIOD;
//	TIM_TimeBaseStructure.TIM_Prescaler = PUMP_TIMER_PRESCALER;
//	TIM_TimeBaseInit(PUMP_TIMERE, &TIM_TimeBaseStructure);
//	TIM_ClearFlag(PUMP_TIMERE, TIM_IT_Update);
//	TIM_ITConfig(PUMP_TIMERE, TIM_IT_Update, ENABLE);
//	TIM_Cmd(PUMP_TIMERE, DISABLE);

//	TIM_TimeBaseStructure.TIM_Period = PUMP_TIMER_PERIOD;
//	TIM_TimeBaseStructure.TIM_Prescaler = PUMP_TIMER_PRESCALER;
//	TIM_TimeBaseInit(PUMP_TIMERF, &TIM_TimeBaseStructure);
//	TIM_ClearFlag(PUMP_TIMERF, TIM_IT_Update);
//	TIM_ITConfig(PUMP_TIMERF, TIM_IT_Update, ENABLE);
//	TIM_Cmd(PUMP_TIMERF, DISABLE);
}
...
Bool PumpTimer_RegisterHandle(TimerHandle Handle, void* obj)
{
    ...
    else if (PumpTimerE_Handle == NULL)
	{
	   PumpTimerE_Handle = Handle;
	   s_timerEObj = obj;
	   TRACE_CODE("\n###########Start TIMERE: ");
	}
	else if (PumpTimerF_Handle == NULL)
	{
	   PumpTimerF_Handle = Handle;
	   s_timerFObj = obj;
	   TRACE_CODE("\n###########Start TIMERD: ");
	}
    else
...
Bool PumpTimer_CancelRegisterHandle(void* obj)
{
    ...
    else if (obj == s_timerEObj)
	{
		PumpTimerE_Handle = NULL;
		s_timerEObj = NULL;
		TRACE_CODE("\n###########Discon TIMERE: ");
	}
	else if (obj == s_timerDObj)
	{
		PumpTimerF_Handle = NULL;
		s_timerFObj = NULL;
		TRACE_CODE("\n###########Discon TIMERF: ");
	}
    else
...
//void PUMP_TIMERE_IRQHANDLER(void)
//{
//    if ((PUMP_TIMERE->DIER & TIM_IT_Update)
//            && (PUMP_TIMERE->SR & TIM_IT_Update))  //检查更新中断发生与否
//    {
//        // 清更新中断标志
//        PUMP_TIMERE->SR &= (uint16_t) (~((uint16_t) TIM_IT_Update));
//        PumpTimerE_Handle(s_timerEObj);
//    }
//}

//void PUMP_TIMERF_IRQHANDLER(void)
//{
//    if ((PUMP_TIMERF->DIER & TIM_IT_Update)
//            && (PUMP_TIMERF->SR & TIM_IT_Update))  //检查更新中断发生与否
//    {
//        // 清更新中断标志
//        PUMP_TIMERF->SR &= (uint16_t) (~((uint16_t) TIM_IT_Update));
//        PumpTimerF_Handle(s_timerFObj);
//    }
//}
